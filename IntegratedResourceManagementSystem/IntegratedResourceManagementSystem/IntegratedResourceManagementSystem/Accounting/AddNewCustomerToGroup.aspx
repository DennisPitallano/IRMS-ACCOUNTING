<%@ Page Title="" Language="C#" MasterPageFile="~/Accounting/Accounting.Master" AutoEventWireup="true" CodeBehind="AddNewCustomerToGroup.aspx.cs" Inherits="IntegratedResourceManagementSystem.Accounting.AddNewCustomerToGroup" %>
			...
			<div class="items-selected" style="width: 370px; margin-bottom: 10px;">
				...
				<table style="">
					<tr>
						...
						<td>
							<asp:TextBox CssClass="modalText" Width="250px" Height="20px" ID="txtGroupName" 
				   ...
				   <asp:TextBox ID="txtSearch" runat="server" CssClass="modalText" autofocus="true" placeholder="Search Outlet"
					...
					<asp:GridView ID="gvCustomers" runat="server" DataSourceID="SqlDataSourceOutlets"
						...
						<Columns>
							...
							<asp:BoundField DataField="CompName" HeaderText="OUTLET NAME" SortExpression="CompName" />
					...
					<asp:SqlDataSource ID="SqlDataSourceOutlets" runat="server" ConnectionString="<%$ ConnectionStrings:IRMSConnectionString %>"
						SelectCommand="SELECT CustInfoEx.CustNo, CustInfoEx.CompName, CustInfoEx.brand,CustInfoEx.MainCustNo
	FROM   CustInfoEx  
	where CustInfoEx.MainCustNo is not null and CustNo NOT IN(SELECT CUSTOMER_INVENTORY_GROUPS.CUSTOMER_NUMBER 
						from CUSTOMER_INVENTORY_GROUPS 
						Where CUSTOMER_INVENTORY_GROUPS.INVENTORY_GROUP_ID=@INVENTORY_GROUP) ">
						<SelectParameters>
							<asp:QueryStringParameter Name="INVENTORY_GROUP" 
								QueryStringField="GroupNumber" />
						</SelectParameters>
					</asp:SqlDataSource>
					<asp:SqlDataSource ID="SqlDataSourceOutletsByBrand" runat="server" ConnectionString="<%$ ConnectionStrings:IRMSConnectionString %>"
						SelectCommand="SELECT CustInfoEx.CustNo, CustInfoEx.CompName, CustInfoEx.brand,CustInfoEx.MainCustNo FROM CustInfoEx where  CustInfoEx.MainCustNo is not null and CustNo NOT IN(SELECT CUSTOMER_INVENTORY_GROUPS.CUSTOMER_NUMBER from CUSTOMER_INVENTORY_GROUPS Where CUSTOMER_INVENTORY_GROUPS.INVENTORY_GROUP_ID=@INVENTORY_GROUP) AND ([brand] = @brand)">
						<SelectParameters>
